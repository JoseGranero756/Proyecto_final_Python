...
class VistaReview(tk.Frame):
    def __init__(self,master = None, controlador = None):
        ...
        self.comentario_entry = tk.Entry(self)
        ...
        self.radio1_animo = tk.Radiobutton(self, text="Bueno", variable=self.animo, value=0,command= lambda: print("Buena"))
        self.radio1_animo.grid(row=3, column=1, padx=5, pady=5)

        self.radio1_animo = tk.Radiobutton(self, text="Malo", variable=self.animo, value=1, command= lambda: print("Mala") )
        self.radio1_animo.grid(row=4, column=1, padx=5, pady=5)

        self.boton_guardar = tk.Button(
            self,
            text="Guardar",
            command = lambda: self.controlador.guardar(100, 2, 2, int(self.calificacion_entry.get()), self.comentario_entry.get(), self.animo.get())
        )
